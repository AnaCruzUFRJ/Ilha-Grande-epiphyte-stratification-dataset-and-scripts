...
library(ggplot2) #save the graphic
...
dados<-read.table("composicao_por_zona_semhemiepifitas.csv",header=T,sep=";")
# View(dados)
str(dados)
...
par(family = "arial", size = 8)

par(mar = c(5, 4, 4, 4) + 0.1, xpd = TRUE)
plot(PCoA2 ~ PCoA1, data = grafico, type = "n",
     xlab = "PCoA1 (17.8%)", ylab = "PCoA2 (13.8%)", family = "arial", size = 8)
...
legend(.50, .5, legend = c("1","2", "3", "4", "5"), pch = 21, pt.bg = colores, title = "Zones", bty = "n")

# save the graphic
tiff(filename = "pcoa.jpeg", width= 20, height =15, unit="in", res= 300)
