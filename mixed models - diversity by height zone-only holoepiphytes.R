...
setwd("D:/Arquivos/Documents/ANA 2021/TESE/Tese 2021/Manuscritos/manuscrito 1/Submissão/correções do Zotz")

ZONAS<-read.table("riqueza.por.zona.sem.hemiepifitas.csv",header=T,sep=";")
head(ZONAS)
# View(ZONAS)
...
p_media_desvio + theme(axis.title = element_text(size = 8, family="arial"),
          axis.text = element_text(size = 8, family="arial"))
# para salvar o grafico
ggsave("media_richenss.jpeg", units="in", dpi=300, width = 5, height = 5 )
############################################################################################################################ 
...
# each of the plot separately# 

##############################################################################################################################################
Parc<-c(FlorestaSubmontana2="PLot 5", FlorestaTerrasBaixas="Plot 3",FlorestaSubmontana1="PLot 4",Restinga1="Plot 1", Restinga2="Plot 2")
...
ZONAS<-read.table("Abundancia.por.zona.sem.hemiepifitas.csv",header=T,sep=";")
head(ZONAS)
# View(ZONAS)
ZONAS$Zones<-as.factor(ZONAS$Zones)
str(ZONAS)
...
abd_media_desvio + theme(axis.title = element_text(size = 8, family="arial"),
                       axis.text = element_text(size = 8, family="arial"))

ggsave("media_sd_abd.jpeg", units="in", dpi=900, width = 5, height = 5 ) # to save the graphic # 
